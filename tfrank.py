...
TRAIN=../data/dedup/train_letor.txt && \
VALI=../data/dedup/vali_letor.txt && \
TEST=../data/dedup/test_letor.txt && \
rm -rf $OUTPUT_DIR && \
python3 tfrank.py \
--train_path=$TRAIN \
--vali_path=$VALI \
...
flags.DEFINE_integer("num_features", 150, "Number of features per document.")
    ...
    with open(path, "rt") as f:
        for line in f:
            ...
            for k, v in six.iteritems(features):
                assert k in feature_map, "Key {} not founded in features.".format(
                    k)
...
def make_score_fn():
    ...
    def _score_fn(unused_context_features, group_features, mode, unused_params,
        ...
        with tf.name_scope("input_layer"):
            ...
            tf.summary.scalar("input_sparsity",
