...
cpdef longest_common_subsequence(X, Y):
    """Compute and return the longest common subsequence length
    X, Y are list of strings"""
    cdef:
        unsigned int m = len(X) 
        unsigned int n = len(Y)
        unicode xx = <unicode>X
        unicode yy = <unicode>Y

        # np.ndarray[np.int32_t, ndim=2] C = np.zeros([m+1, n+1], dtype=np.int32)
        np.ndarray[np.uint16_t, ndim=2] C = np.zeros([m+1, n+1], dtype=np.uint16)
        unsigned int i, j
